...
FriendlyChat.prototype.initFirebase = function() {
  // Add Firebase SDK shortcuts
  this.auth = firebase.auth();
  this.database = firebase.database();
  this.storage = firebase.storage();

  // Initiate Firebase and listen to authentication state changes
  this.auth.onAuthStateChanged(this.onAuthStateChanged.bind(this));
};

// Loads chat messages history and listens for upcoming ones.
FriendlyChat.prototype.loadMessages = function() {
  // Get reference of our messages table in database
  this.messagesRef = this.database.ref("messages");

  // Turn off/remove all previous event listeners
...
FriendlyChat.prototype.saveMessage = function(e) {
  ...
  if (this.messageInput.value && this.checkSignedInWithMessage()) {
    ...
    this.messagesRef.push({
      ...
      // Clear message field and reset SEND button state
      FriendlyChat.resetMaterialTextfield(this.messageInput);
      this.toggleButton();
    }.bind(this)).catch(function (error) {
    console.error("Error while writing new message to Firebase database", error);
    });
...
FriendlyChat.prototype.signIn = function(googleUser) {
  // Sign into Firebase using popup authentication with Google as identity provider
...
FriendlyChat.prototype.checkSignedInWithMessage = function() {
  // Return true if user is signed into Firebase
